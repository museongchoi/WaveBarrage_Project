...
**문제점 01**: 특정 무기가 1레벨 상황에서 해당 무기 카드 선택 시 효과가 적용되지 않는 문제가 발생.
...
**문제점 01** : C++ 로 변환하며 리슨서버의 구조를 파악하고 적용하는데 어려움이 있었습니다.
- 클라이언트이자 서버인 리슨서버의 구조를 파악하고 서버와 클라이언트 데이터 전달, 동기화 방식을 공부하여 진행했습니다.
...
**문제점 01** : 플레이어 공격을 위해 로컬 플레이어의 입력을 처리 해야하는데, 리슨 서버의 구조상 서버 폰이 클라이언트 폰과 동시에 존재하는 예외 처리를 하지 못하여 문제가 발생하였습니다.
- 주요 로직을 폰에서 처리하는게 아닌 컨트롤러에서 서버로 요청하여 서버에서 처리하는 구조로 진행을 했다면 더 좋은 결과가 있었을거라 생각합니다.
...
**문제점 01: 오토모드에서** 가장 가까운 몬스터를 찾는 알고리즘보다 더 최적화된 알고리즘을 적용할 것입니다.
...
**문제점 01 (해결)** : 여러 무기의 레벨별 데이터를 관리하기 위한 문제 발생.
- **해결 방안**
    - 데이터 테이블 기반 설계로 코드 유지보수성과 확장성 향상.
...
**문제점 01 (해결)** : 언리얼 엔진의 비헤이비어 트리를 사용해 각각의 몬스터가 가까운 플레이어를 찾고 네트워크로 위치를 동기화 한다면 몬스터가 가장 많은 경우 프레임 드랍이 발생하였습니다.
- **최적화 방안**
