...
### **05. 캐릭터 및 무기 시스템**
- **무기의 회전 동기화** : 서버에 공격 요청을 보낸 뒤 서버에서 공격 방향으로 캐릭터를 회전시킨 후 각 클라이언트에 해당 공격 방향을 동기화합니다.
...
**문제점**: 캐릭터의 전용무기가 1레벨 -> 2레벨 스텟 적용이 되지 않는 문제 발생.
- **해결 방안**: 일반 무기와 전용 무기를 같은 객체로 취급하여 상속 구조에서 문제가 발생한것으로 예상됩니다. 전용무기는 게임이 시작된 후 기본적으로 가지고 있는 무기이며, 일반 무기는 카드를 선택해야 가질 수 있는 무기이므로 서로 다른 객체로 분리하거나 선택한 카드의 무기의 스텟을 적용시키는 로직에서 조건을 수정하는 것으로 해결.
